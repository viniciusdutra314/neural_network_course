...
Para verificar o desempenho do modelo, foram geradas 20.000 imagens com o mesmo nível de ruído das de treinamento: 10.000 de cada classe.
...
Para realizar a classificação, aplica-se a função sinal `sgn` a esse valor e a mudança de pesos e víés é feita de forma a minimizar o erro quadrático $E = 1/2 (t-z)^2$. Dessa forma, os gradientes em relação aos pesos e ao viés são
...
foi feito até que o modelo classificasse corretamente todos os dados de treinamento ou até um número máximo de épocas.
...
#figure(
  image("relatorio_print_resultado.jpeg", width: 43%),
